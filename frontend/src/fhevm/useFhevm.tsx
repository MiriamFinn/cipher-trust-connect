...
export function useFhevm(parameters: {
  ...
  console.log("[useFhevm] Hook called with:", { 
    provider: typeof provider === "string" ? provider : provider ? "Eip1193Provider" : "undefined",
    chainId, 
    enabled 
  });
  ...
  // Main useEffect
  useEffect(() => {
    console.log("[useFhevm] Main useEffect triggered, _isRunning:", _isRunning, "_providerRef.current:", _providerRef.current);
    
    // is _providerRef.current valid here ?
    // even if the first useEffect is rendered in the same render-cycle ?
    if (_isRunning === false) {
      // cancelled
      console.log("[useFhevm] Cancelled (enabled=false)");
      if (_abortControllerRef.current) {
        _abortControllerRef.current.abort();
        _abortControllerRef.current = null;
      }
    ...
    if (_isRunning === true) {
      // Allow "sepolia" string to pass through (it's a special marker, not undefined)
      if (_providerRef.current === undefined && typeof provider !== "string") {
        console.log("[useFhevm] Provider is undefined and not a string, returning early");
      ...
      console.log("[useFhevm] Starting instance creation, Provider ref:", _providerRef.current, "Provider param:", provider);
